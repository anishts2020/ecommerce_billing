...
function SalesInvoice() {
  ...
  const netTotal = Number((grandTotal + stichingTotal - toNumber(discount) + toNumber(tax)).toFixed(2));
 ...
 /* -------------------
  Make sure these helper functions stay in your file
   - toNumber, fmt, etc
   - keep your other code as-is
   - Just replace buildStitchingPayload and saveInvoice with below
   ------------------- */

// build stitching payload (now includes item_temp_id so backend can map reliably)
const buildStitchingPayload = (customerId, customerName) => {
  const payload = [];
  items.forEach((it) => {
    const productId = it.product_id ?? null;
    const productName = it.name ?? "";
    const itemTempId = it.id; // important: temp id from UI row
    it.stitchings.forEach((s) => {
      payload.push({
        ...
        rate: Number(s.rate),
        item_temp_id: itemTempId, // NEW: helps backend link to invoice item
      });
    });
  });
  return payload;
...
// save invoice (use item.temp_id inside items)
const saveInvoice = async (customerIdFromServer) => {
  if (!items || items.length === 0) {
    setAlert({
      isOpen: true,
      title: "No items",
      message: "Add at least one product before generating invoice.",
      type: "error",
      onClose: () => setAlert((a) => ({ ...a, isOpen: false })),
    });
    return;
  ...
  // items payload now includes temp_id so backend can map to created rows
  const payloadItems = items.map((item) => ({
    temp_id: item.id,
    product_id: item.product_id,
    product_name: item.name,       // ADD THIS
  ...
  try {
    const res = await axios.post(`${API_BASE}/sales-invoices`, payload);
    ...
    setAlert({
      ...
      onClose: () => {
        ...
        navigate("/sales-invoice"); // ✅ redirect
      },
      onConfirm: () => {
        setAlert((a) => ({ ...a, isOpen: false }));
        navigate("/sales-invoice"); // ✅ redirect
    ...
    // show server validation or error payload (very important)
    console.error("Failed to save invoice:", err?.response?.data ?? err);
    const serverMsg = err?.response?.data?.message ?? "Failed to save invoice. See console.";
    setAlert({
      isOpen: true,
      title: "Save Error",
      message: serverMsg,
...
const handleCustomerSave = () => {
  ...
  request
    .then((res) => {
      const customerId = existingId ? existingId : res.data.id ?? res.data.data?.id ?? null;
      ...
      setAlert({
        ...
        message: existingId ? "Customer updated successfully!" : "Customer added successfully!",
        type: "success",

        onConfirm: () => {
          setAlert((a) => ({ ...a, isOpen: false }));
          saveInvoice(customerId); // invoice save
        },
